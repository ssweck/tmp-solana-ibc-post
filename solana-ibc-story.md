...
Implementing [the Inter-Blockchain Communication protocol
(IBC)](https://www.ibcprotocol.dev/) is however not a trivial task.
With all the encapsulation, block header signatures, state proofs
etc. which IBC core needs to handle, the contract can easily go past
those limits.  In this post I go through some of the things we had to
do to bring IBC onto Solana.


## The Guest blockchain

Fist of, Solana doesn’t meet IBC’s host requirements listed in [ICS 24
document](https://github.com/cosmos/ibc/blob/main/spec/core/ics-024-host-requirements/README.md).
It offers neither state proofs nor introspection.  The former is
required to create membership and non-membership proofs.  The latter
to preform IBC handshake when establishing an IBC connection.  This is
...
It describes technical difficulties of developing a complex smart
...
of the standard which make sense for Cosmos introduce challenge when
working with a completely different ledger like Solana.


## Logging

Quite early in the development we’ve run into memory problems.  32 KiB
may have been enough to code Super Mario Bros. but nowadays, heap this
size is quite limiting.  Especially when using frameworks not
necessarily designed with such limit in mind.

When writing software, a common quick way to troubleshoot issues is to
add some logging.  Logs offer some visibility to program’s internal
...
In the end, even though we’re using Anchor for Solana IBC
...
usage](https://github.com/coral-xyz/anchor/pull/2691) has been merged
into Anchor¹ but at this point we’ve already moved on and Solana IBC
...
it’s a feature makes sense in the context of Solana.  Smart contract
...
For cases where more sophisticated memory management strategy is
...
Note that `solana_program::entrypoint` and `anchor_lang::program`
...
failure caused by duplicate allocator definition.  To fix that,
`custom-heap` Cargo feature needs to be enabled in program’s crate.
The following snippet included in project’s `Cargo.toml` file dose
that:
...
of the smart contracts called in that transaction.  `RequestHeapFrame`
is exactly what doctor ordered for one with higher heap size
requirements.
...
In `BumpAllocator` presented above this corresponds to the use of
...
1. Caller of the smart contract (i.e. sender of the transaction),
   ...
   account in the instruction invoking to the smart contract.
2. Smart contract identifies that account in set of accounts it has
   been given.
3. Smart contract iterates over instructions to look for a call to
...
heap smart contract author needss to:
...
signatures is to call [Ed25519 native
...
global variables.  We knew exactly what kind of state we needed to
keep and how large it is.  Since we controlled heap allocations, we
simply reserved portion of the heap for any mutable global object we
needed.

The allocator was already doing it with its own state.  Extending that
...
to check multiple in a single transaction.  In an optimistic case
...
2. For each group, send Solana transaction which calls the Ed25519
   ...
   look at all the verified signatures and store commitment it
...
4. Finally call `sigverify` to free up the account.  (Caller pays for
   storage of the account so freeing recovers the tokens spent on
   storage rent).
...
interface for interacting with Ed25519 program?  Sure, there’s
...
expects the private key pair as argument.  A quite useless interface
except in situations where sender of the transaction is the one
...
support for reading the request from an account rather than
...
are hard to break through when necessary.  [`anchor_lang::program`
...
conflict with function we’ve defined.  Unfortunately with Anchor
...
⁶ Hence the term ‘chunking’ was born.  Since chronologically we’ve
developed `write-account` program before `sigverify` we started
